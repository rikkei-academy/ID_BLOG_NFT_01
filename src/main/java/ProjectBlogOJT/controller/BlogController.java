...
import ProjectBlogOJT.payload.response.PopularArticles;
...
//        blogUpdated.setBlogCreateDate(blog.getBlogCreateDate());
    ...
    public List<PopularArticles> listBlog() {
        List<Blog> listBlogs = blogService.findAll();
        List<PopularArticles> popularArticlesList = new ArrayList<>();
        for (Blog blog : listBlogs) {
            int countLike = likeService.countLike(blog.getBlogID());
            PopularArticles popularArticles = new PopularArticles(
                    blog.getBlogID(),
                    blog.getBlogTitle(),
                    blog.getBlogContent(),
                    blog.getBlogImage(),
                    blog.getBlogCreateDate(),
                    blog.isBlogStatus(),
                    countLike
            );
            popularArticlesList.add(popularArticles);
        }
        Collections.sort(popularArticlesList, new Comparator<PopularArticles>() {
            @Override
            public int compare(PopularArticles u1, PopularArticles u2) {
                return u2.getLike() - u1.getLike();
            }
        });
        return popularArticlesList;
    }
